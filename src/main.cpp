...
int loadConfig() {
  try {
    ...
    std::cerr << "Could not open config file /etc/raspindi.conf" << std::endl;
    return (EXIT_FAILURE);
...
void mirrored_rotation(VideoOptions* options) {
  ...
  if (hflip) transform = libcamera::Transform::HFlip * transform;
  if (vflip) transform = libcamera::Transform::VFlip * transform;

  bool ok;
  libcamera::Transform rotation =
      libcamera::transformFromRotation(_getValue("rotation", 0), &ok);
...
static void event_loop(RPiCamEncoder& app) {
  ...
  std::unique_ptr<Output> output = std::unique_ptr<Output>(new NdiOutput(
      options, _getValue("neopixel_path", "/tmp/neopixel.state")));
