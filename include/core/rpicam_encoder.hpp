...
class RPiCamEncoder : public RPiCamApp {
 public:
  ...
  bool EncodeBuffer(CompletedRequestPtr& completed_request, Stream* stream) {
    ...
    // Setting this means that we won't skip frames that are missing sync
    // metadata even though we had previously achieved sync. This could happen
    // if the control algorithm doesn't run every frame.
    sync_achieved_ = true;
#endif
