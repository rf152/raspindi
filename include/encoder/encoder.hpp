...
class Encoder {
 public:
  ...
  // encoder has finished with an input buffer, so the application can re-use
  // it.
  ...
  virtual void EncodeBuffer(int fd, size_t size, void* mem,
                            StreamInfo const& info, int64_t timestamp_us) = 0;
...
typedef Encoder* (*EncoderCreateFunc)(VideoOptions* options,
