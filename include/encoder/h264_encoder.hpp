...
class H264Encoder : public Encoder {
 ...
 private:
  ...
  // This thread just sits waiting for the encoder to finish stuff. It will
  // either:
  // * receive "output" buffers (codec inputs), which we must return to the
  // caller
  // * receive encoded buffers, which we pass to the application.
  void pollThread();

  // Handle the output buffers in another thread so as not to block the encoder.
  // The application can take its time, after which we return this buffer to the
  // encoder for re-use.
  void outputThread();
